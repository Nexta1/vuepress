...
|------|------------------------|-------------------------------|
| 定义   | 对表中的数据行进行物理排序的索引       | 不对数据行进行物理排序的索引                |
| 数量   | 每个表只能有一个聚簇索引           | 每个表可以有多个非聚簇索引                 |
| 叶子节点 | 存储实际的数据行               | 存储指向数据行的指针                    |
| 查询性能 | 适合范围查询和顺序访问            | 适合单行查找和满足特定条件的查询              |
| 数据排序 | 数据行按照聚簇索引的顺序存储         | 数据行在物理上没有特定的顺序                |
| 更新开销 | 插入、删除、更新数据可能需要重新排序数据行  | 插入、删除、更新数据只需要更新非聚簇索引，无需重排序数据行 |
| 示例   | InnoDB引擎中的主键索引是聚簇索引    | InnoDB引擎中的辅助索引是非聚簇索引          |

索引可以加快查询速度的主要原因是它提供了一种更高效的数据访问方式，减少了数据库系统需要扫描整个表的开销。
